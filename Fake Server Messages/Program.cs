  ...
  class Program
  {
    ...
    private static void OnChatSender(GameChatEventArgs args)
    {
      ...
      if (args.Message.StartsWith(".msg"))
      {
        ...
        if (allX == true)
        {
          string message = string.Format("/all {0}{1}  {2}  {3}", premsg, new string('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa' + sender.Length), sender, fakemsg);
          Game.Say(message);
          args.Process = false;
        ...
        {
          string message = string.Format("     {0}{1}  {2}  {3}", premsg, new string(' ', 70 + sender.Length), sender, fakemsg);
          Game.Say(message);
          args.Process = false;
